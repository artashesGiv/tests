...
import isAnyElementInViewport from "@/shared/lib/isAnyElementInViewport";
...
export default class SectionAnimation extends Vue {
    ...
    @Inject('$browsers') $browsers!: BrowserProviderValue
    ...
    get isAnimationComplete(): boolean {
        return Boolean((this.$vnode.componentOptions?.tag && !completeSections.includes(this.$vnode.componentOptions.tag)) && !this.$browsers.IE)
    }
